...
for user_entry in user_data:
    ...
    tweets_data = [(tweet["Testo"], tweet["Data"],
    tweet["Retweet"],tweet["Favorite"]) for tweet in tweets]
    user_info = {
        "Categoria": user_entry["Categoria"],
        "Tweets": tweets_data,
        "Posts": user_entry["Posts"],
        "Followers": user_entry["Followers"],
        "Friends": user_entry["Friends"],
        "Likes": user_entry["Likes"]
    }
...
for user, data in users.items():
    ...
    minhash.update(str(data['Posts']).encode('utf-8'))
    minhash.update(str(data['Followers']).encode('utf-8'))
    minhash.update(str(data['Friends']).encode('utf-8'))
    minhash.update(str(data['Likes']).encode('utf-8'))
    for tweet_data in data["Tweets"]:
        data_tweet = tweet_data[1]
        num_retweet = tweet_data[2]
        num_likes = tweet_data[3]
        ...
        minhash.update(str(num_retweet).encode('utf-8'))
        minhash.update(str(num_likes).encode('utf-8'))
...
for query_data in query_data_list:
    ...
    query_tweets = [(tweet["Testo"], tweet["Data"],
    tweet["Retweet"],tweet["Favorite"]) for tweet in query_data["Tweets"]]
    query_user_info = {
    ...
    query_minhash.update(str(query_user_info['Posts']).encode('utf-8'))
    query_minhash.update(str(query_user_info['Followers']).encode('utf-8'))
    query_minhash.update(str(query_user_info['Friends']).encode('utf-8'))
    query_minhash.update(str(query_user_info['Likes']).encode('utf-8'))
    for tweet_data in query_user_info["Tweets"]:
        ...
        query_minhash.update(str(num_retweet).encode('utf-8'))
        query_minhash.update(str(num_likes).encode('utf-8'))
